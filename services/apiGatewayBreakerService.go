...
func (r *GatewayBreakerService) InsertBreaker(bk *GatewayBreaker) *GatewayResponse {
	...
	var addURL = r.Host + "/rs/gwBreakerSuper/add"
...
func (r *GatewayBreakerService) UpdateBreaker(bk *GatewayBreaker) *GatewayResponse {
	var rtn = new(GatewayResponse)
	var upURL = r.Host + "/rs/gwBreakerSuper/update"
...
func (r *GatewayBreakerService) ResetBreaker(bk *GatewayBreaker) *GatewayResponse {
	var rtn = new(GatewayResponse)
	var upURL = r.Host + "/rs/gwBreakerSuper/reset"
...
func (r *GatewayBreakerService) GetBreaker(urlID string, routeID string, clientID string) *GatewayBreaker {
	...
	var gURL = r.Host + "/rs/gwBreakerSuper/get/" + urlID + "/" + routeID + "/" + clientID
	//fmt.Println(gURL)
...
func (r *GatewayBreakerService) GetBreakerStatus(routeURLID string, clientID string) *Status {
	var rtn = new(Status)
	var gURL = r.Host + "/rs/gwBreakerSuper/status/" + routeURLID + "/" + clientID
	//fmt.Println(gURL)
	req, rErr := http.NewRequest("GET", gURL, nil)
	if rErr != nil {
		fmt.Print("request err: ")
